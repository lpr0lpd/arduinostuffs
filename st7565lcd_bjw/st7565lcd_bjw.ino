...
//bjw  glcd.display(); // show splashscreen
//bjw  delay(200);
  glcd.clear();
/*  int b = 1;
  int c = 60;
while (1) {
  ...
  glcd.setpixel(b, c, BLACK);
  glcd.setpixel(b+1, c+1, BLACK);
  glcd.display();        // show the changes to the buffer
  delay(250);
  glcd.setpixel(b+1, c+1, WHITE);
  glcd.setpixel(b, c, WHITE);
  glcd.display();
    if (c == 63 && b == 126) {
    b = 1;
    c = 1;
...
//bjw  testdrawline();
       testdrawchar();
//bjw  glcd.display();       // show the lines
//bjw  delay(2000);
//bjw  glcd.clear();
...
//bjw  glcd.fillcircle(32, 32, 10, BLACK);
//bjw  glcd.display();
//bjw  delay(2000);
//bjw  glcd.clear();
...
//bjw  glcd.clear();
  ...
  glcd.drawstring(0, 0, "Brian Williams");
  glcd.display();
  delay(2000);
  glcd.clear();

  // draw a bitmap icon and 'animate' movement
//bjw  testdrawbitmap(logo16_glcd_bmp, LOGO16_GLCD_HEIGHT, LOGO16_GLCD_WIDTH);
}
